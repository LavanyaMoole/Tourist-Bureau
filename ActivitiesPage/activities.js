...
function getActivitiesNames(category){
...
function init(){
    ...
    activitiesCategory.onchange = selectedCategoryChanged;
...
function populateCategories(){
    ...
    for(let category of Categories){
        ...
        activitiesCategory.appendChild(option)

    }
}
function filterActivitiesUsingCategory(category){
...
function selectedCategoryChanged(){
    ...
    const activitiesCategory = document.getElementById("activitiesCategory").value;
    console.log(activitiesCategory)
    const activitiesFiltered=filterActivitiesUsingCategory(activitiesCategory);
    console.log(activitiesFiltered);
    let activitiesList=document.getElementById("activitiesList");
    activitiesList.innerHTML="";
    for(let filteredActivity of activitiesFiltered){

        const option=new Option(filteredActivity.name);
    ...
    activitiesList.onchange = function () {
        let selectedActivityName = activitiesList.value;
...
function displayActivity(activity) {
        ...
        activityLocation.innerText = activity.location;
        activityPrice.innerText = `$${activity.price.toFixed(2)}`;

        if(activity.price > 0 ) {
            document.getElementById("eticket-form").style.display = "block";
        } else {
            document.getElementById("eticket-form").style.display = "none";
        }
...
function populateDetails(selectedACtivity){
    const activityName=document.getElementById("activityName");
    activityName.innerText;
}
